 ...
 *
 * Test suite for CIA_BYTEARRAY data structure in C Programming Language.
 */
...
int main(int argc, char **argv)
{
  ...
  CIA_DEBUG_ENTER("Running test suite for CIA_BYTEARRAY data structure")

  test_status = acceptence_test();
  if (test_status != CIA_SUCCESS)
  {
    CIA_ERROR("Failed acceptence test for CIA_BYTEARRAY data structure")
    suite_status = EXIT_FAILURE;
  }

  CIA_DEBUG_EXIT("Finished test suite for CIA_BYTEARRAY data structure")
